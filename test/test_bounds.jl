...
s = IDQ.interval_arithmetic_sign_search(f, box, 1e-2)
@test !(s == 0 || s == 1 || s == -1)

f(x) = x[1] + 2
box = IntervalBox(-1..1, 1)
@test IDQ.interval_arithmetic_sign_search(f, box, 1e-2) == 1
...
f(x) = x[1]
box = IntervalBox(-1..1, 1)
@test IDQ.interval_arithmetic_sign_search(f, box, 1e-2) == 0
