...
function allequal(v1,v2;tol=1e-14)
...
@test allequal(quad1.points,IDQ.extend([1.0],1,quad2.points))
@test allequal(quad1.weights,w0*quad2.weights)
...
@test allequal(quad_ext.points,testp)
@test allequal(quad_ext.weights,testw)
...
@test allequal(surf_quad.points,p)
@test allequal(surf_quad.weights,w0)
...
@test allequal(quad.points,p2)
@test allequal(quad.weights,w2)
...
@test allequal(p,quad.points)
@test allequal(quad1d.weights,quad.weights)
