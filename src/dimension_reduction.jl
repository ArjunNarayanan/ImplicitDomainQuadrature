function check_unique_roots(roots)
    @assert all([r.status .== :unique for r in roots])
end
...
"""
    sign_conditions_satisfied(funcs,xc,sign_conditions)
...
function quadrature(F,sign_conditions,lo,hi,
    ...
    for j in 1:length(roots)-1
        ...
        if sign_conditions_satisfied(F,xc,sign_conditions)
...
function quadrature(F::Vector, sign_conditions::Vector{Int},
    int::Interval{T}, quad1d::ReferenceQuadratureRule{N,T}) where {N,T}

    return quadrature(F, sign_conditions, int.lo, int.hi, quad1d)
end

function quadrature(F::Vector, sign_conditions::Vector{Int},
    height_dir::Int, lo::T, hi::T, x0::AbstractVector{T}, w0::T,
    quad1d::ReferenceQuadratureRule{N,T}) where {N,T}
...
function quadrature(F::Vector, sign_conditions::Vector{Int},
    height_dir::Int, int::Interval{T}, x0::AbstractVector{T},
    w0::T, quad1d::ReferenceQuadratureRule{N,T}) where {N,T}

    return quadrature(F,sign_conditions,height_dir,int.lo,int.hi,x0,w0,quad1d)
end

function quadrature(F::Vector, sign_conditions::Vector{Int},
    height_dir::Int, lo::T, hi::T, x0::AbstractMatrix{T},
    w0::AbstractVector{T}, quad1d::ReferenceQuadratureRule{N,T}) where {N,T}
...
function quadrature(F::Vector, sign_conditions::Vector{Int},
    height_dir::Int, int::Interval{T}, x0::AbstractMatrix{T},
    w0::AbstractVector{T}, quad1d::ReferenceQuadratureRule{N,T}) where {N,T}
...
"""
function surface_quadrature(F::InterpolatingPolynomial,
    height_dir::Int, lo::T, hi::T, x0::AbstractVector{T}, w0::T) where {T}
...
function surface_quadrature(F::InterpolatingPolynomial,
    height_dir::Int, int::Interval{T}, x0::AbstractVector{T}, w0::T) where {T}
...
"""
function isSuitable(height_dir::Int, P::InterpolatingPolynomial,
    box::IntervalBox; order::Int = 5, tol::Float64 = 1e-3)
...
function quadrature(P::InterpolatingPolynomial{N,NF,B,T},
    sign_condition::Int, surface::Bool, box::IntervalBox{2,T},
    quad1d::ReferenceQuadratureRule{NQ,T}) where {B<:TensorProductBasis{2}} where {N,NF,NQ,T}
